...
{  //Please refer to documentation for public variables
    ...
    public bool buttonLeftClick;
    public bool buttonRightClick;
    public bool buttonJumpClick;
    public bool buttonDownClick;
    void init()
    {
    ...
    void Awake()
    {
        jumpBtn.onClick.AddListener(ButtonJumpClick);
        leftBtn.onClick.AddListener(ButtonLeftClick);
        rightBtn.onClick.AddListener(ButtonRightClick);
        DownBtn.onClick.AddListener(ButtonDownClick);
    ...
    void Update()
    {
        ...
        { //What to do in Playing state
            ...
            if (EnableJump&&(Input.GetKeyDown(KeyCode.UpArrow) || AndroidControls() == 1 || buttonJumpClick==true) && CanJump == true && dead == false)
            {
                buttonJumpClick = false;
                jump = true;
                CanJump = false;
            ...
            if (EnableSlide&&(Input.GetKeyDown(KeyCode.DownArrow) || AndroidControls() == 2 || buttonDownClick==true) && dead == false && isSliding == false)
            {
                buttonDownClick = false;
                if(!DisableAnimations)
    ...
    { //Slot movement are the movement of the Player in horizontal direction
      //All sideways movement calculations are here.
        ...
        if (dead == false && isRotating == false && canChangeSlot == true)
        {
            ...
            {
                if (AndroidControls(1) == 3 || Input.GetKeyDown(KeyCode.RightArrow) || buttonRightClick==true)
                {
                ...
                if (AndroidControls(1) == 4 || Input.GetKeyDown(KeyCode.LeftArrow) || buttonLeftClick == true)
                {
                    RaycastHit hit;
                    if (Physics.Raycast(transform.position, -transform.right, out hit, SideWayEnemyCheckDist, EnemyAndWallLayer))
                    {
            ...
            else if (TrackType == TrackTypeEnum.FreeHorizontalMovement)
            {
                ...
                else if (AndroidControls(1) == 4 || Input.GetKeyDown(KeyCode.LeftArrow) || buttonLeftClick==true)
                {//If there is a wall on the side then stumble. If already stumbling then die
                    HorizontalMoveValue = -2f;
                    ishaking = true;
                    StartCoroutine("Shake");
    ...
    public void Rotate(Vector3 pos, ref bool canrot, int rotType = 0, Transform target = null, Transform target1 = null)
    {
        if (dead == false && CurrentGameState == GameState.Playing)
        {
            if (rotType == 0)
            {//both side turn
                if ((Input.GetKeyDown(KeyCode.LeftArrow) || AndroidControls(1) == 4) || AutoTurn == true || buttonLeftClick==true)
                {
                    buttonLeftClick = false;
                    if (target != null)
                    {
                ...
                else if ((Input.GetKeyDown(KeyCode.RightArrow) || AndroidControls(1) == 3) || AutoTurn == true || buttonRightClick==true)
                {
                    buttonRightClick = false;
                    if (target != null)
                    {
            ...
            if (rotType == 1)
            { //Right turn
                if ((Input.GetKeyDown(KeyCode.RightArrow) || AndroidControls(1) == 3) || AutoTurn == true || buttonRightClick==true)
                {
                    buttonRightClick = false;
                    if (target != null)
                    {
